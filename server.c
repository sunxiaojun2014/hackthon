...
char *line_process(char *in_start, char *out_start, int count) {
	if (in_start == NULL || out_start == NULL || count == 0) {
		return out_start;
	}
	...
	if (count >= 4) { // count 包含换行符 
		// 去除size/3的数据量
		i = (count - 1) / 3;
		memcpy(in_start + i, in_start + 2*i, count - 2*i);
	}

	int len = count - i;
	reverse(in_start, in_start + len - 2); // 逆序不包含换行符
	memcpy(out_start, in_start, len);

	return (out_start+len);
}
	...
	while (nleft > 0) {
		if (*buf != '\0') {
			...
			if (*buf == '\r' || *buf == '\n') {
				...
				start = line_process(buf - byte_count + 1, start, byte_count);
				...
				md[line_num].byte_count = byte_count;
...
void *send_brick(void *arg) {
	...
	int id = nc->id;
	// 按照线程数进行分割(每个线程数处理相同行数的数据)
	int line_count = (id <= THREAD_NUM) ? line_total / THREAD_NUM : line_total - (line_total / THREAD_NUM) * (THREAD_NUM - 1);
	int line_start = (id - 1) * (line_total / THREAD_NUM);
	int byte_count = md[line_start+line_count-1].byte_count;
	char *data = file_buf + line_start; 
	int line = 0;
	int request_num;
	int nr;
	...
	length_encode(byte_count, writebuf+9);
	printf("line_count:%d\n", line_count);
    ...
    for ( ;line < line_count; ) {
		if (request_num = readn(connfd, readbuf, 1024) <= 0) {
		...
		for (int i = 0; i < request_num; i++) {
			...
			if (writen(connfd, writebuf, nr+4) <= 0) {
				print_err("server write error:", errno);
				close(connfd);
				return NULL;
			}
